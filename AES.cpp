...
// ✅ ĐÃ SỬA: Dùng tham chiếu unsigned char (&state)[4][4]
...
void ShiftRows(unsigned char (&state)[4][4]) {
    ...
    // Hàng 3: dịch vòng trái 3 byte (tuong duong d?ch ph?i 1 byte)
    temp = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = state[3][0];
...
// Hàm phụ thực hiện phép nhân trong trường GF(2^8)
...
void MixColumns(unsigned char (&state)[4][4]) {
    ...
    for (int c = 0; c < 4; ++c) {
        // Sao chép cột ra biến tạm
        for (int r = 0; r < 4; ++r) {
            temp[r] = state[r][c];
        }

        // Thực hiện phép nhân ma trận (đã được định nghĩa sẵn)
...
// ✅ ĐÃ SỬA: Dùng tham chiếu const unsigned char (&state)[4][4]
void in(const unsigned char (&state)[4][4])
{
    for (int i = 0; i < 4; i++) // Vòng lặp hàng
    {
        for (int j = 0; j < 4; j++) // Vòng lặp cột
        {
            // Thêm định dạng hexa và ép kiểu sang int
