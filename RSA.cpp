...
void generateKeys(long long &n, long long &e, long long &d) {
    ...
    srand(time(0));
    do {
        p = rand() % 50 + 100; // Chọn p trong khoảng [50, 99]
    } while (!isPrime(p));

    do {
        q = rand() % 50 + 100; // Chọn q trong khoảng [100, 149]
    } while (!isPrime(q) || p == q);
    ...
    do {
        e = rand() % (phi_n - 2) + 2; // Chọn e trong khoảng [2, phi_n-1]
    } while (gcd(e, phi_n) != 1);
    
    cout << "Khoa cong khai Kp = (e, n) = (" << e << ", " << n << ")" << endl;

    d = modInverse(e, phi_n);
    cout << "Khoa bi mat Ks = (d, p, q) = (" << d << ", " << p << ", " << q << ")" << endl;
}
...
int main() {
    ...
    cout << "Ban ma can giai ma: " << encrypted_message << endl;d
